...
/* Arm Neon has not move bit mask(sse: PMOVMSKB)
 * 
 * The following is the Arm neon implementation
 * But it is too slow on Arm64
 *
...
int32_t move_byte_mask_neon(uint8x16_t input)
{
    ...
    const char __attribute__ ((aligned (16))) xr[16] = {-7,-6,-5,-4,-3,-2,-1,0,-7,-6,-5,-4,-3,-2,-1,0};
    uint8x16_t mask_and = vdupq_n_u8(0x80);
    int8x16_t mask_shift = vld1q_s8(xr);

    input = vandq_u8(input, mask_and);
    input = vshlq_u8(input, mask_shift);

    input = vpaddq_u8(input,input);
    input = vpaddq_u8(input,input);
    input = vpaddq_u8(input,input);

    //return (vgetq_lane_u16((uint16x8_t)input, 0) & 0xffff);
